...
void receivefile(char* receivedfile, int new_fd)
{
	...
	//char GotFileSize_previous[1024];
	char GotFileSize_temp[1024];
	long fileSize;
	long SizeCheck = 0;
	long memstart = 0;
	...
	while(SizeCheck < 1024){
	  ...
	  long read_rec = recv(new_fd, GotFileSize_temp, 1024 - SizeCheck, 0);//, 0 ); <-----returns horrible value
	  //int read_rec = recv(new_fd, pointer_GotFileSize, 1024 - SizeCheck, 0);
	  printf("received at file size %d\n",read_rec);
	  SizeCheck += read_rec;
	  printf("received at file size %d\n",read_rec);
	  printf("received sizecheck %d\n",SizeCheck);
	  
	  //snprintf(GotFileSize,sizeof GotFileSize,"%s%s",GotFileSize,GotFileSize_temp);
	  
	  //memcpy(GotFileSize+memstart,GotFileSize_temp,SizeCheck); //where,what,size
	  memcpy(GotFileSize+memstart,GotFileSize_temp,read_rec); //where,what,size
	  
	  printf("\nGotFilesize_INIT: %s\nSizecheck_init:%d\nRead_rec:%d\n\n\n",GotFileSize,SizeCheck,read_rec);
	}
